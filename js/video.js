...
VideoJS.options = {
  ...
  flashKeyForwardProgressIncrement: 10,

  playbackNotSupportedErrorMessage: 'Video Playback Not Supported',

  missingFlashPluginMessage: 'Missing or out of date Flash Player plugin detected. The latest ' +
      'version of the <a href="http://get.adobe.com/flashplayer/">player</a> may support the ' +
      'video format that is attempting to be played.',
...
_V_.Component = _V_.Class.extend({
  ...
  init: function(player, options){
    ...
    this.addEvent("focus", _V_.proxy(this, this.focusStylingFocus));
    this.addEvent("blur", _V_.proxy(this, this.focusStylingBlur));
    this.addEvent("click", _V_.proxy(this, this.focusStylingClick));
    this.addEvent("keydown", _V_.proxy(this, this.focusStylingKeypress));
  },
  ...
  // This following functionality deals with adding a class
  // (focusClass) to the component when it receives
  // focus via the keyboard not mouse. This will allow
  // focus styling of the element for keyboard accessibility
  // without affecting the design if the mouse is used). The
  // event hnadlers are named so as not to interfere with
  // subclasses that will likely have event handlers for at
  // least some of the same events.
  focusStylingFocus: function(){
    setTimeout(_V_.proxy(this, this.applyFocusClass), 200);
  },

  focusStylingBlur: function(){
    this.removeClass(this.focusClass);
  },

  focusStylingClick: function(){
    this.mouseClick = true;
  },

  focusStylingKeypress: function(){
    this.mouseClick = false;
  },

  applyFocusClass: function(){
    if (!this.mouseClick) {
  ...
  getVideoTagSettings: function(){
    ...
    if (this.tag.hasChildNodes()) {
      for (var c,i=0,j=this.tag.childNodes;i<j.length;i++) {
        ...
        } else if (c.nodeName.toLowerCase() == "track") {
          options.tracks.push({
            src: c[getAttribute]("src"),
  ...
  src: function(source){
    ...
    if (source instanceof Array) {
      ...
      if (sourceTech) {
        ...
        // Hide the video tag so it does not overlay any .playbackNotSupportedMessage
        // els, preventing their links from being clickable
        this.tag.style.display = "none";
        errorMessageEl = _V_.createElement('p', {className: 'playbackNotSupportedMessage'});
        errorMessageEl.innerHTML = this.options.playbackNotSupportedErrorMessage;
        this.el.appendChild(errorMessageEl);
        _V_.log("No compatible source and playback technology were found.")
        if (!_V_.flash.isSupported()) {
            errorMessageEl = _V_.createElement('p', {className: 'missingFlashPluginMessage'});
            errorMessageEl.innerHTML = this.options.missingFlashPluginMessage;
            this.el.appendChild(errorMessageEl);
            _V_.log("Missing or out of date Flash Player plugin detected.")
        }
      }
...
_V_.Slider = _V_.Component.extend({
  ...
  update: function(){
    ...
    newBarWidth = _V_.round(barProgress * 100, 2);
    bar.el.style.width = newBarWidth + "%";
    formattedProgress = _V_.round(progress * 100, 2);

    this.el.setAttribute("aria-valuenow", formattedProgress);
  },
...
_V_.SeekBar = _V_.Slider.extend({
  ...
  stepForward: function(){
	  var time,
        flashIncrement = this.player.options.flashKeyForwardProgressIncrement;

    if (this.player.techName === "flash") {
      time = this.player.currentTime() + flashIncrement;
      time = time > this.player.duration() ? this.player.duration() - 0.1 : time;
    } else {
      time = this.player.currentTime() + 1;
    }
  ...
  stepBack: function(){
	  var time = this.player.currentTime() - 1;

    if (this.player.techName === "flash" && time < 0) {
      time = 0;
    }
...
_V_.LoadProgressBar = _V_.Component.extend({
  ...
  createElement: function(){
    return this._super("div", {
      ...
      innerHTML: this.buildInnerHTML(0)
    });
  },

  update: function(){
    var width = _V_.round(this.player.bufferedPercent() * 100, 2)

    if (this.el.style) { this.el.style.width = width + "%"; }
    this.el.innerHTML = this.buildInnerHTML(width);
  },

  buildInnerHTML: function(val){
    return '<span class="vjs-control-text">Loaded: ' + val + '%</span>';
  }
...
_V_.PlayProgressBar = _V_.Component.extend({
  ...
  createElement: function(){
    return this._super("div", {
      ...
      innerHTML: '<span class="vjs-control-text">Progress Bar</span>'
    });
  }
...
_V_.SeekHandle = _V_.Component.extend({
  ...
  createElement: function(){
    return this._super("div", {
      ...
      innerHTML: '<span class="vjs-control-text">Progress Handle</span>'
    });
  }
...
_V_.VolumeLevel = _V_.Component.extend({
  ...
  createElement: function(){
    return this._super("div", {
      ...
      innerHTML: '<span class="vjs-control-text">Volume Bar</span>'
    });
  }
...
_V_.VolumeHandle = _V_.Component.extend({
  ...
  createElement: function(){
    return this._super("div", {
      ...
      innerHTML: '<span class="vjs-control-text">Volume Handle</span>'
      // tabindex: 0,
